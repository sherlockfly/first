...
function refreshTab(){
	...
	self.parent.$('#tabs').tabs('update', {
	  tab : currTab,
	  options : {
	   content : createFrame('orderindex',url)
	  }
	 });
}

function refreshGrid(cfg,title){
	var refresh_tab =  self.parent.$('#tabs').tabs('getSelected'); //获得当前tab
	//var refresh_tab = cfg.tabTitle?$('#main_tab').tabs('getTab',cfg.tabTitle):$('#main_tab').tabs('getSelected');//获取到选定的tab  
    if(refresh_tab && refresh_tab.find('iframe').length > 0){  
	    ...
	    var refind = refresh_doc.$('.pagination-load').parent().parent('a');//获取到刷新元素class $('.pagination-load') 
	    //var reholle = refresh_doc.getElementById("holle");//获取到具体的元素ID	      
	    //reholle.click();//找到元素ID时，进行点击事件刷新。 
	    if(refind==null){ 
...
function addTab(subtitle, url, icon) {
	if (!$('#tabs').tabs('exists', subtitle)) {
		$('#tabs').tabs('add', {
			title : subtitle,
			content : createFrame('orderiframe',url),
			closable : true,
			icon : icon
